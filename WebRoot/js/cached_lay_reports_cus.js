  ...
  $.fn.ProSelect = function(options) {
    if (this.length > 0) {
      ...
      var defaults = {
        ...
        custom: true,
        callback: '',
        minDate: null, // add by chenjincai My product-->应用趋势选择控件有用，禁止选择8.1, 之前的日期，传递的是Date Object, 然后传给datepicker使用
    ...
    this.bindSubmit = function(settings){
        settings.panel.find('.submit').unbind('click').bind('click',function(){
            ...
            if(global.pickedEndDay != ''){
                ...
                _track_userEvent('New function monitoring_时段对比', num + 'day', num);
            }
