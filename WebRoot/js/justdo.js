	...
	window.IM = window.IM || {
		...
		init: function() {
			...
			if (200 == resp.code) { // 初始化成功
				...
				console.log("Error code：170002,Error code描述" + resp.msg);
			} else {
...
//						$("#pop_group_" + groupId + ">.modal-footer").append(saveChange);
		...
		DO_getCity: function() {
			var arr = new Array();
			arr[0] = "东城,西城,Chongwen,Xuanwu,朝阳,丰台,石景山,海淀,门头沟,房山,通州,顺义,昌平,大兴,平谷,怀柔,密云,延庆"
			arr[1] = "黄浦,Luwan,徐汇,长宁,静安,普陀,闸北,虹口,杨浦,闵行,宝山,嘉定,浦东,金山,松江,青浦,Nanhui,奉贤,崇明"
			arr[2] = "和平,东丽,河东,西青,河西,津南,南开,北辰,河北,武清,Red Bridge,Tanggu,Hangu,Dagang,宁河,静海,宝坻,蓟县"
			arr[3] = "万州,涪陵,渝中,大渡口,江北,沙坪坝,九龙坡,南岸,北碚,Maxell,Double bridge,渝北,巴南,黔江,长寿,綦江,潼南,铜梁,大足,荣昌,Bishan,梁平,城口,丰都,垫江,武隆,忠县,开县,云阳,奉节,巫山,巫溪,石柱,秀山,酉阳,彭水,江津,合川,永川,南川"
			arr[4] = "石家庄,邯郸,邢台,保定,张家口,承德,廊坊,唐山,秦皇岛,沧州,衡水"
			arr[5] = "太原,大同,阳泉,长治,晋城,朔州,吕梁,忻州,晋中,临汾,运城"
			arr[6] = "呼和浩特,包头,乌海,赤峰,Hulun Buir Union,阿拉善盟,Zhelimumeng,兴安盟,Wulanchabu Union,锡林郭勒盟,Bayannaoer Union,Yikezhaomeng"
			...
			arr[30] = "乌鲁木齐,石河子,克拉玛依,伊犁,Bayangol,昌吉,Kirgiz,Bo Ertala,吐鲁番,哈密,喀什,和田,阿克苏"
			arr[31] = "香港"
			arr[32] = "澳门"
			arr[33] = "台北,高雄,台中,台南,屏东,南投,云林,新竹,彰化,苗栗,嘉义,花莲,桃园,宜兰,基隆,台东,金门,Matsu,澎湖"
		...
		_cancelTakePic: function() {
			...
			var onErr = function(obj) {
				console.log("Error code：" + obj.code + "; Error code描述：" + obj.msg);
			};
			RL_YTX.photo.cancel();
		},
			...
			RL_YTX.getHistoryMessage(getHistoryMessageBuilder,
				function(obj) {
					...
					for (var i = 0; i < obj.length; i++) {
						...
						var str = '';
						if (msg.msgType == 1) { //1:文本消息 2：语音消息 3：视频消息 4：图片消息 5：位置消息 6：file   msg.msgFileName; //消息file名
