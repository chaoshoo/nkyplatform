		...
		onSelect : function(file) {
			...
			for (var n in this.queueData.files) {
				...
				if (queuedFile.uploaded != true && queuedFile.name == file.name) {
					var replaceQueueItem = confirm('file "' + file.name + '" Existing.确定覆盖之前的file?');
					if (!replaceQueueItem) {
			...
			if ($.inArray('onSelect', settings.overrideEvents) < 0) {
				...
				$('#' + settings.queueID).append('<div id="' + file.id + '" class="uploadify-queue-item">\
					<div class="cancel">\
						<a href="#" onclick="jQuery(\'#' + settings.id + '\').uploadify(\'cancel\', \'' + file.id + '\');">X</a>\
					</div>\
			...
			if ($.inArray('onSelectError', settings.overrideEvents) < 0) {
				switch(errorCode) {
					...
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						this.queueData.errorMsg += 'file "' + file.name + '" 超过规定file大小 (' + settings.fileSizeLimit + ').';
						break;
