...
function refreshSearchTag(){
	...
	$("<option value='' >--Please select--</option>").appendTo("#group-tag-list_select")//添加下拉框的option
	$.ajax({ 
          type : "post", 
...
function messagesTmpGrid(id){ 
	...
	$.ajax({ 
          ...
          success : function(data){ 
        	  if(data.code==1 && data.categories != null && data.categories.length >0){
      			$("<option value='0'>--Please select a template--</option>").appendTo("#msg-msg_template_select")//添加下拉框的option
      			for(var o in data.categories){  
      				var obj = {};
...
function messagesTmpGrids(){ 
	...
	$.ajax({ 
          ...
          success : function(data){ 
        	  if(data.code==1 && data.categories != null && data.categories.length >0){
      			$("<option value='0'>--Please select a template--</option>").appendTo("#msgs-msg_template_select")//添加下拉框的option
      			for(var o in data.categories){  
      				var obj = {};
      				obj.id = data.categories[o].id;
      				obj.msg_type = data.categories[o].msg_type;
		...
		$.post("vip/inspectlatest/"+rowInfo.CARD_CODE+".json",function(data){
			if(data.code==1){
				...
				if(data.categories != null && data.categories.length >0){
					...
					htmls += "<tr><td>Name</td> <td>Detection coding</td> <td>检测Name</td> <td>Detection value</td> <td>Detection time</td> <td>operation</td> </tr>";
					for(var o in data.categories){  
...
function openNew(){
	...
	if(initAreaFlag && areaJsons != []  && areaJsons.length > 0){
    	$("<option value='0'>--Please select--</option>").appendTo("#un_AREA")//添加下拉框的option
...
function openedit(id){
	...
	var rowInfo =  $('#base_table').datagrid('getSelected');
	if(initAreaFlag && areaJsons != []  && areaJsons.length > 0){
    	$("<option value='0'>--Please select--</option>").appendTo("#u_AREA")//添加下拉框的option
		for(var o in areaJsons){  
	...
	$('#base_table').datagrid({
		...
		columns:[[
		  ...
		  /*   {field:'WEIGHT',title:'体重',width:100}, 
		    ...
		    `gm` varchar(250) DEFAULT NULL COMMENT 'History of allergy',
...
<body class="easyui-layout">
	...
	<div id="user_inspect_detail_dialog" data-options="closed:true,modal:true,title:'Detection details'" style="padding: 5px; width: 850px; height: 450px;">
		...
		<div id="user_inspect_detail_tab" class="easyui-tabs" style="width:810px;height:330px;"> 
		     ...
		     <div title="ECG detection record  " id="last_inspect_ecg_datas">
