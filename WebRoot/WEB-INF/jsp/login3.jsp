	...
	<div class="container" id="loginWindow">
      <form class="form-signin" action="login.html" method="post">
        <h2 class="form-signin-heading">
        	Ning Kang Yuan<span style="color:red"><c:choose><c:when test="${type eq 'doctor'}">Doctor</c:when><c:when test="${type eq 'hospital'}">Hospital</c:when><c:when test="${type eq 'sys'}">Administrators</c:when></c:choose></span>Background login system
        </h2>
        <div class="login-wrap">
			<%-- <p class="l_ts">${logfail}</p>
			<div class="info">
				<input class="login-input username" id="userMail" name="username" placeholder="enter one user name"  />
            ...
            <input type="text" class="form-control" id="userMail" name="username" 	required="true" 	onvalidation="onUserNameValidation" placeholder="enter one user name" autofocus>
