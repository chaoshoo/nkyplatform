	...
	<tr style="width: 100%;height: 50px">
		...
		<td width="50%">
			<c:if test="${data.frontPage < data.currentPage }">
				<a class="w3-btn w3-round-xxlarge" href="<%=basePath%>/vipInspectData/chart/${data.cardCode }/${data.inspectCode }/${data.detailCode }/${data.frontPage }.html">上一页</a> 
			</c:if>
			<c:if test="${data.nextPage > data.currentPage }">
				<a class="w3-btn w3-round-xxlarge" href="<%=basePath%>/vipInspectData/chart/${data.cardCode }/${data.inspectCode }/${data.detailCode }/${data.nextPage }.html">下一页</a>
			</c:if>
		</td>
		<td width="25%"></td>
	</tr>
